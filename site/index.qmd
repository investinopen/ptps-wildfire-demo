...
::: {.screenshots layout-ncol=3}
[![Browser extension](img/screenshots/extension.webp){fig-alt="The browser extension's modal over a missing EPA page, offering the rescued data"}](fallbacks.qmd#browser-extension)

[![Dataset rescue statuses](img/screenshots/rescue-status.webp){fig-alt="Summary table of how many example data sources are live and archived"}](analysis/rescue-status.qmd)

[![Risk calculation](img/screenshots/risk.webp){fig-alt="Map of Wyoming's fire weather zones, shaded by risk score"}](analysis/risk.ipynb)

[![Wildfire overlap](img/screenshots/fire-overlap.webp){fig-alt="Map of fire perimeters across Wyoming since 1984, darker where they overlap"}](analysis/fire_overlap.ipynb)

[![Detailed firefighter map](img/screenshots/detailed-map.webp){fig-alt="Detailed map of Gold Hill, Colorado, with buildings colored by wildfire risk, roads, trails, and house numbers"}](firefighter-maps/detailed/index.html)

[![Simplified firefighter map](img/screenshots/simplified-map.webp){fig-alt="Road graph of Gold Hill, Colorado, colored by steepness, with dead ends and sharp turns marked"}](firefighter-maps/simplified/index.qmd)
:::

- **[Fallbacks](fallbacks.qmd):** tools for finding a backup when a source URL goes away.
  - [HTTP proxy](fallbacks.qmd#http-proxy)
  - [Python package](fallbacks.qmd#python-package)
  - [Browser extension](fallbacks.qmd#browser-extension)
- **Analysis**
  - **[Dataset rescue statuses](analysis/rescue-status.qmd):** checks whether the example wildfire data sources are still up, and whether they've been rescued to the [Wayback Machine](https://web.archive.org/) or the [Data Rescue Project Portal](https://portal.datarescueproject.org/). Updates daily.
  - **[Risk calculation](analysis/risk.ipynb):** ranks a state's fire weather zones by combining burn probability, burn history, Red Flag Warnings, and active fire detections.
  - **[Wildfire overlap](analysis/fire_overlap.ipynb):** maps every fire perimeter in a state since 1984, so places that have burned more than once stand out.
- **Firefighter maps:** printable maps for small/rural fire departments.
  - **[Detailed](firefighter-maps/detailed/index.html):** combines building footprints, fuel load, burn probability, fire hydrants, roads, driveways, and trails.
  - **[Simplified](firefighter-maps/simplified/index.qmd):** draws the roads as a graph, to scale by driving distance, with steep grades, sharp turns, dead ends, and house numbers marked.
...
- If someone is looking for a dataset that's been disrupted, how do they find the (appropriate) backup?
  - This is where the [fallbacks](fallbacks.qmd) come in.
    - [HTTP proxy](fallbacks.qmd#http-proxy)
    - [Python package](fallbacks.qmd#python-package)
    - [Browser extension](fallbacks.qmd#browser-extension)
...
- Most solutions assume a high-speed internet connection on a modern device. How can we make relevant data available to those who (urgently) need it but don't have internet or power?
  - Disaster scenarios
  - Remote areas
  - This is what the [maps for firefighters](firefighter-maps/detailed/index.html) is about.
- What data would be useful to operational folks but is too hard for them to access/use?
  ...
  - This is also what the [maps for firefighters](firefighter-maps/detailed/index.html) is about.
...
- [On building for the long run, not just the crisis](https://www.linkedin.com/pulse/building-long-run-just-crisis-kaitlin-thaney-txdhc/)
