...
A simplified map for navigation ([#22](https://github.com/investinopen/ptps-wildfire-demo/issues/22)): the roads around a rural town drawn as a graph of intersections and the roads between them, each drawn with its length proportional to its driving distance, colored by its steepest grade, and with sharp turns, dead ends, gates, and addresses marked. Roads come from OpenStreetMap via [OSMnx](https://osmnx.readthedocs.io/), and elevation from [Mapterhorn](https://mapterhorn.com/)'s terrain tiles. It's the notebook [`firefighter-maps/simplified/index.ipynb`](firefighter-maps/simplified/index.ipynb); set `PLACE_NAME`/`CENTER` in its configuration cell to move it. Its code is in [`ptps_wildfire_demo/simplified_map/`](../ptps_wildfire_demo/simplified_map/), with tests in [`tests/test_simplified_map.py`](../tests/test_simplified_map.py).
